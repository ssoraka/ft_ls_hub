...
void	print_list_flag_l(t_dlist *list)
{
	...
	ft_putstr(list->mode);
	//ft_putstr(ft_bytes_to_bits((char *)(&list->stats->st_mode), 2, "__"));
	//printf("   %d\n", major(list->stats->st_rdev));
	//printf("   %d\n", minor(list->stats->st_rdev));
...
char	*ft_return_mode(short st_mode, char *link)
{
	...
	if ((st_mode & 0x4000) != 0 && (st_mode & 0x2000) != 0)
		answer[0] = 'b';
	else if ((st_mode & 0x4000) != 0)
		answer[0] = 'd';
	else if ((st_mode & 0x1000) != 0)
		answer[0] = 'p';
	else if ((st_mode & 0x2000) != 0)
		answer[0] = 'c';
	if (link && link[0] != '\0')
		answer[0] = 'l';
	return (answer);
...
t_dlist	*ft_create_sort_list(t_dlist **begin_list, char *name, char *path, struct stat *stats)
{
	...
	if (name[0] == '\0')
	{
		ft_pseudoprintf(MSG_ERROR, "fts_open", 0, 0);
		exit(0);
	}
